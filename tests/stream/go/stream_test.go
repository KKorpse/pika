...
var _ = Describe("Stream Commands", func() {
	...
	Describe("streams", func() {
		...
		PIt("XADD with MAXLEN option and the '=' argument", func() {
			Expect(client.Del(ctx, "mystream").Err()).NotTo(HaveOccurred())
			for i := 0; i < 1000; i++ {
				if rand.Float64() < 0.9 {
		...
		PIt("XADD with NOMKSTREAM option", func() {
			Expect(client.Del(ctx, "mystream").Err()).NotTo(HaveOccurred())
		...
		PIt("XADD with MINID option", func() {
			Expect(client.Del(ctx, "mystream").Err()).NotTo(HaveOccurred())
		...
		PIt("XTRIM with MINID option", func() {
			Expect(client.Del(ctx, "mystream").Err()).NotTo(HaveOccurred())
		...
		PIt("XTRIM with MINID option, big delta from master record", func() {
			Expect(client.Del(ctx, "mystream").Err()).NotTo(HaveOccurred())
			Expect(client.XAdd(ctx, &redis.XAddArgs{Stream: "mystream", ID: "1-0", Values: []string{"f", "v"}}).Err()).NotTo(HaveOccurred())
		...
		It("Non blocking XREAD with empty streams", func() {
			...
			result, err := client.Do(ctx, "XREAD", "STREAMS", "s1", "s2", "0-0", "0-0").Result()
